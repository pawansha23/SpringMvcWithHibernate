...
public class AccountController {
	...
	public ModelAndView getPdf(@PathVariable("accno") String accno) {
		...
		ModelAndView modelView = new ModelAndView("accountList");
		int tempAccNo = Integer.valueOf(accno);
		System.out.print(accno);
		Map<String, AccountVO> data = new HashMap();
		data.put("oneAccount", accountService.getAccount(tempAccNo));
		Map<String, List<AccountVO>> allData = new HashMap();
		allData.put("accounts", accountService.showAccounts());
		modelView.addAllObjects(data);
		modelView.addAllObjects(allData);

		return modelView;
	}

	@RequestMapping(value = "/showPdfPage/{accno}", method = RequestMethod.GET)
	public ModelAndView getPdfPage(@PathVariable("accno") String accno) {
